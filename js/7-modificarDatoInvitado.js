      ...
      for (let i = 0; i < sistemaPersona.personas.length; i++) {
        ...
        if (unaPersona.cedula === cedulaInvitado) {
          if (unaPersona.validado === false) {
            ...
            document.querySelector("#pMensajeDatosInvitado").innerHTML =
              "Los datos estan disponibles para modificar";
            ...
            document.querySelector(
              "#apellidoModificado"
            ).value = `${unaPersona.apellido}`;
            document.querySelector(
              "#edadModificado"
            ).value = `${unaPersona.edad}`;
            document.querySelector(
              "#departamentosModificado"
            ).value = `${unaPersona.departamento}`;
            document.querySelector(
              "#ocupacionModificado"
            ).value = `${unaPersona.ocupacion}`;
...
document
  .querySelector("#modificarDatosInvitado")
  .addEventListener("click", tomarDatosModificados);

function tomarDatosModificados() {
  console.log("entra a la funcion que modifica persona");

  let nombreInvitadoModificada =
    document.querySelector("#nombreModificado").value;
  let apellidoInvitadoModificada = document.querySelector(
    "#apellidoModificado"
  ).value;
  let edadInvitadoModificada = document.querySelector("#edadModificado").value;
  let departamentoInvitadoModificada = document.querySelector(
    "#departamentosModificado"
  ).value;
  let ocupacionInvitadoModificada = document.querySelector(
    "#ocupacionModificado"
  ).value;
  ...
  if (!validarNombreOApellido(nombreInvitadoModificada)) {
    ...
    document.querySelector("#pMensajeDatosInvitado").innerHTML =
      "Debe seleccionar una ocupacion";
  } else {
    sistemaPersona.modificarPersona(
      cedulaAModificar,
      nombreInvitadoModificada,
      apellidoInvitadoModificada,
      edadInvitadoModificada,
      departamentoInvitadoModificada,
      ocupacionInvitadoModificada
    );
...
function agregarDepartamentosInv() {
  //despliega los departamentos automaticamente
...
function agregarOcupacionInv() {
  //despliega las ocupaciones automaticamente
