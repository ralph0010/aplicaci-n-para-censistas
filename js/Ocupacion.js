...
export class Ocupacion {
  ...
  export function retornarOcupacionPorTipo(tipoIngresado){
    ...
    ocupacionRetorno = ocupaciones.find(({tipo}) => 
      Number(tipo) == Number(tipoIngresado) 
    );
    if(ocupacionRetorno === null || ocupacionRetorno === undefined) throw new Error ("No se encontró la ocupación");
  ...
  function tipoEsValido(tipo){
    if(tipo < valorMinimoOcupacion || tipo> valorMaxOcupacion) throw new Error("La ocupación ingresada no es válida")
  }
